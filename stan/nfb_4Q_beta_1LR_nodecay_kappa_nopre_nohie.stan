...
transformed parameters {
  ...
  for(S in 1: nSubject){
    for (Se in 1:nSession[S]){ 
      alpha_norm[S,Se]= alpha_subject_raw[S] + (alpha_Visit * SessionIndex[S,Se]);
      beta_norm[S,Se]=beta_subject_raw[S] + (beta_Visit * SessionIndex[S,Se]);
      ...
      kappa[S,Se] = kappa_subject_raw[S] + (kappa_Visit * SessionIndex[S,Se]);
    }
  }
