...
parameters { 
  ...
  vector[2] alpha_subject_raw[nSubject];
  vector[2] beta_subject_raw[nSubject];
  //vector[nSubject] tau_subject_raw;
  //vector[nSubject] omega_subject_raw;
  vector[2] pers_subject_raw[nSubject];
  //vector[nSubject] per_raw;
  vector[2] kappa_subject_raw[nSubject];
...
transformed parameters {
  ...
  for(S in 1: nSubject){
    for (Se in 1:nSession[S]){ 
      alpha_norm[S,Se]=alpha_m + (alpha_group_s * alpha_subject_raw[S,Se]) + (alpha_Visit * SessionIndex[S,Se]);
      beta_norm[S,Se]=beta_m + (beta_group_s * beta_subject_raw[S,Se]) + (beta_Visit * SessionIndex[S,Se]);
      //tau_norm[S,Se]=tau_m + (tau_group_s * tau_subject_raw[S]) + (tau_Visit * SessionIndex[S,Se]);
      //pers[S,Se] = pers_m + (pers_group_s[Se] * pers_subject_raw[S,Se]) + (pers_Visit * SessionIndex[S,Se]);
      kappa[S,Se] = kappa_m + (kappa_group_s * kappa_subject_raw[S,Se]) + (kappa_Visit * SessionIndex[S,Se]);
    }
  }
...
model {
  ...
  alpha_Visit ~ normal(0,5);
  beta_Visit ~ normal(0,5);
  kappa_Visit ~ normal(0,5);
