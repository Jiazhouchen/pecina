data {
  ...
  int Infusion_f [nSubject,2,maxTrialN]; 
...
parameters { 
  ...
  real<lower=0,upper=1> alpha_diff_m; //this is also the alpha learning rate if it's only one 
  //Inverse Temperature
  real beta_diff_m;
  //Reward Sensitivity
  //real tau_m;
  ...
  vector[nSubject] alpha_Visit;
  vector[nSubject] beta_Visit;
  vector[nSubject] kappa_Visit;
  //real omega_Visit;
  //real tau_Visit;
  //real pers_Visit;
  
  real<lower=0>alpha_diff_s;
  real<lower=0>beta_diff_s;
  real<lower=0>kappa_diff_s;
  ...
  vector[nSubject] alpha_diff_raw;
  vector[nSubject] beta_diff_raw;
  vector[nSubject] kappa_diff_raw;
...
transformed parameters {
  ...
  alpha_norm[S,Se]=alpha_m + (alpha_group_s * alpha_subject_raw[S]) + ( (alpha_diff_m+(alpha_diff_s * alpha_diff_raw[S])) * SessionIndex[S,Se]);
  beta_norm[S,Se]=beta_m + (beta_group_s * beta_subject_raw[S]) + ( (beta_diff_m+(beta_diff_s * beta_diff_raw[S])) * SessionIndex[S,Se]);
  kappa[S,Se] = kappa_m + (kappa_group_s * kappa_subject_raw[S]) + ( (kappa_diff_m+(alpha_diff_s * kappa_diff_raw[S])) * SessionIndex[S,Se]);
    }
  }
...
model {
  ...
  real Q[nSubject,2,maxTrialN+1,4];
  ...
  alpha_diff_m ~ normal(0,2.5);
  beta_diff_m ~ normal(0,5);
  kappa_diff_m ~ normal(0,5);
  
  alpha_diff_s ~ cauchy(0,1);
  beta_diff_s ~ cauchy(0,1);
  kappa_diff_s ~ cauchy(0,1);
      ...
      if(nTrial[s,se]!=0){
        ...
        for (t in 1:nTrial[s,se]){
          ...
          if(missing_choice_exp[s,se,t]==0){
            ...
            choice ~ bernoulli_logit( ((Q[s,se,t,Infusion_f[s,se,t]]+ kappa[s,se])  * beta[s,se]) );
            //prev_choice = choice? 1:-1 ; //1 if choice NO, -1 if choice YED
            
            //Decay everything;
            for (xe in 1:4) {
            Q[s,se,t+1,xe] = Q[s,se,t,xe] * 0.99;
            }
            //Except for the one getting udpated, Update the value
            delta[s,se,t] = (Trial_Feedback[s,se,t]) - Q[s,se,t,Infusion_f[s,se,t]];
            Q[s,se,t+1,Infusion_f[s,se,t]] = Q[s,se,t,Infusion_f[s,se,t]] + (alpha[s,se]*delta[s,se,t]);
      ...
      if(nTrial[s,se]!=0){
        ...
        for (t in 1:maxTrialN){
          ...
          if(missing_choice_exp[s,se,t]==0){
            ...
            //choice yes or no
            choice = ExpRat[s,se,t]==1;
            log_lik[s,se,t] = bernoulli_logit_lpmf(choice | (Q[s,se,t,Infusion_f[s,se,t]]+ kappa[s,se])  * beta[s,se]  );
            //prev_choice = choice? 1:-1 ; //1 if choice NO, -1 if choice YED
